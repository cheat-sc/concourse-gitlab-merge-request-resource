...
use git2::{ RemoteCallbacks, FetchOptions, Cred, Oid };
use std::path::Path;
use gitlab::Gitlab;
use gitlab::api::{ projects::{ self, merge_requests }, Query };
use url::Url;
use std::{ io, fs::File };
use serde_json;
...
fn main() -> Result<()> {
	...
	if {
		...
		cb.credentials(|_, _, _| {
			Cred::userpass_plaintext("oauth2", &input.source.private_token)
		});
		...
		let repo = builder.fetch_options(fo)
	...
	let file = File::create(
		Path::new(&args.directory).join(".merge-request.json")
